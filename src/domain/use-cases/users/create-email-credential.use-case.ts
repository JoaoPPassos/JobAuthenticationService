...
export class CreateEmailCredentialUseCase {
  ...
  async execute(userId: string, data: { email: string; password: string }): Promise<PublicEmailCredential> {
    const encryptedPassword = this.encryption.encrypt(data.password);
    return this.credentialsRepo.create({ id_user: userId, email: data.email, password: encryptedPassword });
  }
}
